...
$bobinas_pacote = isset($_GET['bobinas_pacote']) ? (int)$_GET['bobinas_pacote'] : 1;
...
error_log("📦 PDF Universal Paquetes - Orden: {$id_orden}, Tipo: {$tipo_producto}, Modo: " .
...
try {
    ...
    if ($esReimpresionLote) {
        ...
        $sql = "SELECT numero_item, id, largura as ancho, metragem as metraje, gramatura, 
                       peso_liquido as peso, fecha_hora_producida, bobinas_pacote, tipo_producto
        ...
        if (isset($stock_data['bobinas_pacote']) && $stock_data['bobinas_pacote'] > 0) {
            $bobinas_pacote = $stock_data['bobinas_pacote'];
        }
    ...
    if (!function_exists('dividirTextoEnLineas')) {
        function dividirTextoEnLineas($pdf, $texto, $ancho_max, $font_size = 6)
        {
    ...
    function generarEtiqueta($pdf, $datos)
    {
        // Extraer datos
        ...
        $pdf->write1DBarcode($barcode_etiqueta, 'C128', 20, 2, 30, 8, 0.4, array(
            'position' => '',
            'border' => false,
            'padding' => 0,
            'fgcolor' => array(0, 0, 0),
        ...
        $qr_data = "AMERICA TNT S.A\n" .
            ...
            "Peso: " . formatearNumero($peso_pacote_kg, 2) . " kg\n" .
            "Dimensiones: " . formatearNumero($ancho_metros, 2) . "m x " . formatearNumero($metraje, 1) . "m\n" .
            "Tipo: " . $tipo_producto . "\n" .
            "www.americatnt.com";

        $pdf->write2DBarcode($qr_data, 'QRCODE,L', 76, 2, 20, 20, array(
            'border' => false,
            'padding' => 0
        ));
        ...
        $pdf->SetXY(20, 13.5);
        $pdf->Cell(60, 4, 'www.americatnt.com', 0, 0, 'L');
        ...
        $pdf->SetXY(2, 18.5);
        $pdf->Cell(35, 4, 'Este pacote contem:', 0, 0, 'R');

        $pdf->SetFont('helvetica', 'B', 8);
        $pdf->SetXY(35, 18.5);
        $pdf->Cell(12, 4, $bobinas_pacote, 0, 0, 'C');

        $pdf->SetFont('helvetica', '', 8);
        $pdf->SetXY(47, 18.5);
        $pdf->Cell(15, 4, 'rolos', 0, 0, 'L');

        $pdf->SetLineWidth(0.3);
        $pdf->Line(3, 23, 73, 23);

        $ancho_disponible = 40;
        $lineas_descripcion = dividirTextoEnLineas($pdf, $descripcion_producto, $ancho_disponible, 4);

        $pdf->SetFont('helvetica', '', 9);
        $y_inicio = 25;
        $altura_linea = 5;

        for ($i = 0; $i < count($lineas_descripcion) && $i < 3; $i++) {
            $y_actual = $y_inicio + ($i * $altura_linea);
            $pdf->SetXY(16, $y_actual);
            $pdf->Cell(68, $altura_linea, $lineas_descripcion[$i], 0, 1, 'C');
        }

        $nueva_y = $y_inicio + (min(count($lineas_descripcion), 3) * $altura_linea) + 2;
        ...
        $col_widths1 = array(11, 30, 18, 17, 18);
        $headers1 = array('LOTE N°', 'COR/COLOR', 'GRAMATURA BASIS WEIGHT', 'N° Rolo/Pacote', 'Etiqueta /LABEL');
        ...
        for ($i = 0; $i < count($headers1); $i++) {
            $pdf->SetLineWidth(0.3);
            $pdf->SetDrawColor(0, 0, 0);
            $pdf->Rect($x_pos, $table_y, $col_widths1[$i], $row_height, 'D');

            $pdf->SetXY($x_pos, $table_y + 0.5);

            if ($i == 2) {
                $pdf->MultiCell($col_widths1[$i], ($row_height - 1) / 2, $headers1[$i], 0, 'C', false, 0);
            } else {
                $pdf->Cell($col_widths1[$i], $row_height - 1, $headers1[$i], 0, 0, 'C');
            }
        ...
        for ($i = 0; $i < count($datos1); $i++) {
            ...
            $pdf->Cell($col_widths1[$i], $row_height - 2, $datos1[$i], 0, 0, 'C');
            $x_pos += $col_widths1[$i];
        }

        $table_y += $row_height + 0;

        $col_widths2 = array(18, 17, 19, 19, 21);
        $headers2 = array('N° ITEM', 'METRAGEM', 'LARGURA ROLO', 'PESO PACOTE', 'DATA');
        ...
        for ($i = 0; $i < count($headers2); $i++) {
            $pdf->SetLineWidth(0.3);
            $pdf->SetDrawColor(0, 0, 0);
            $pdf->Rect($x_pos, $table_y, $col_widths2[$i], $row_height, 'D');

            $pdf->SetXY($x_pos, $table_y + 1);
            $pdf->Cell($col_widths2[$i], $row_height - 2, $headers2[$i], 0, 0, 'C');
        ...
        $datos2 = array(
            ...
            formatearNumero($peso_pacote_kg, 3) . ' KG',
            $fecha_produccion
        );
        ...
        for ($i = 0; $i < count($datos2); $i++) {
            ...
            $pdf->Rect($x_pos, $table_y, $col_widths2[$i], $row_height, 'D');
            $pdf->SetXY($x_pos, $table_y + 1);
            $pdf->Cell($col_widths2[$i], $row_height - 2, $datos2[$i], 0, 0, 'C');
            $x_pos += $col_widths2[$i];
        }

        $pdf->SetLineWidth(0.3);
        $pdf->SetDrawColor(0, 0, 0);
        $pdf->Line(3, $table_y + 7, 97, $table_y + 7);

        $pdf->SetFont('helvetica', 'B', 8);
        $pdf->SetXY(3, $table_y + 8);
        $pdf->Cell($width - 2, 4, 'FABRICADO NO PARAGUAY', 0, 1, 'C');

        $pdf->SetLineWidth(0.3);
        $pdf->Line(3, $table_y + 13, 97, $table_y + 13);

        $pdf->SetFont('helvetica', 'B', 5);
        $pdf->SetXY(3, $table_y + 14);
        $pdf->Cell($width - 2, 3, $texto_validez_pt, 0, 1, 'C');

        $pdf->SetFont('helvetica', 'B', 5);
        $pdf->SetXY(3, $table_y + 17);
        $pdf->Cell($width - 2, 4, $texto_validez_en, 0, 1, 'C');

        $pdf->SetLineWidth(0.3);
        $pdf->Line(3, $table_y + 21, 97, $table_y + 21);
    }

    class EtiquetaUniversalPDF extends TCPDF
    {
        public function Header() {}
        public function Footer() {}
    }

    $pdf = new EtiquetaUniversalPDF('L', 'mm', array(100, 80), true, 'UTF-8', false);
    ...
    if ($esReimpresionLote && isset($itemsLote) && !empty($itemsLote)) {
        foreach ($itemsLote as $index => $item) {
            ...
            $bobinas_pacote_item = $item['bobinas_pacote'] ?: 1;
            ...
            $peso_pacote_kg = $peso_kg * $bobinas_pacote_item;
            ...
            $datosEtiqueta = [
                ...
                'metraje' => $metraje,
                'gramatura' => $gramatura,
                'bobinas_pacote' => $bobinas_pacote_item,
            ...
            generarEtiqueta($pdf, $datosEtiqueta);

            error_log("🏷️ Etiqueta " . ($index + 1) . " generada - Item: {$numero_item}, Peso: {$peso_pacote_kg}kg");
        }

        error_log("✅ PDF en lote completado: " . count($itemsLote) . " etiquetas generadas");
        $nombre_archivo = "Etiquetas_{$tipo_producto}_Orden_{$id_orden}_Items_{$itemDesde}-{$itemHasta}.pdf";
    } else {
        $pdf->AddPage();

        $numero_item = isset($stock_data) ? $stock_data['numero_item'] : 1;
        $numero_lote = str_pad($orden['id'], 7, '0', STR_PAD_LEFT);
        ...
        if (isset($stock_data)) {
            ...
            $peso_kg = ($ancho_metros * $gramatura * $metraje) / 1000;
            $peso_pacote_kg = $peso_kg * $bobinas_pacote;
        ...
        $numero_etiqueta = isset($stock_data) ? $stock_data['id'] : $id_orden;
        ...
        error_log("📄 PDF Universal Paquetes generado - Tipo: {$tipo_producto}, Orden: {$id_orden}, Item: {$numero_item}, Peso paquete: {$peso_pacote_kg}kg, Bobinas: {$bobinas_pacote}");
        $nombre_archivo = "Etiqueta_{$tipo_producto}_Paquete_{$numero_etiqueta}_Item_{$numero_item}_{$bobinas_pacote}rolos_" . formatearNumero($peso_pacote_kg, 1) . 'kg.pdf';
    }

    $pdf->Output($nombre_archivo, 'I');
    exit();
} catch (Exception $e) {
    error_log("💥 Error generando PDF Universal Paquetes: " . $e->getMessage());
