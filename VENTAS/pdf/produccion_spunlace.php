...
// 🆕 FUNCIÓN PARA EXTRAER EL DIÁMETRO DE LA DESCRIPCIÓN
function extraerDiametro($descripcion)
{
    // Patrón para capturar el número seguido de "CM" al final de la cadena
    if (preg_match('/(\d+)\s*CM\s*$/i', $descripcion, $matches)) {
        return $matches[1]; // Retorna solo el número sin "CM"
    }
    return '-'; // Valor por defecto si no encuentra el patrón
}

// 🆕 FUNCIÓN PARA EXTRAER SOLO LA PRIMERA PALABRA DEL ACABADO
function extraerPrimeraPalabra($texto)
{
    if (empty(trim($texto))) {
        return ''; // Si está vacío, devolver vacío
    }

    // Usar strtok para obtener la primera palabra (más eficiente que explode)
    $primeraPalabra = strtok(trim($texto), ' ');
    return $primeraPalabra ?: ''; // Devolver la primera palabra o vacío si no hay nada
}

try {
    // Obtener datos de la orden de producción con spunlace
    ...
    // Ajustar valores por defecto para campos que podrían ser NULL
    ...
    // 🆕 EXTRAER EL DIÁMETRO DE LA DESCRIPCIÓN
    $diametro_extraido = extraerDiametro($orden['descripcion']);

    // 🆕 EXTRAER SOLO LA PRIMERA PALABRA DEL ACABADO
    ...
    $pdf->SetXY(29, 25);
    $pdf->Cell(158, 8, $orden['descripcion'], 0, 0, 'L');
    ...
    header("Location: ../secciones/sectorPcp/produccion.php?error=" . urlencode("Error al generar el PDF: " . $e->getMessage()));
    exit();
}
