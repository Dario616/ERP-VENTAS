...
class PcpRepository
{
    ...
    public function obtenerDevolucionesPcp($filtros = [], $inicio = 0, $limite = 10)
    {
        try {
            ...
            $sql = "SELECT 
            pp.id_venta,
            MIN(pp.fecha_asignacion) as fecha_asignacion,
            v.cliente, 
            v.moneda, 
            v.estado,
            COUNT(DISTINCT pp.id_producto) as productos_devueltos,
            STRING_AGG(DISTINCT pres.descripcion, ', ') as productos_descripcion,
            STRING_AGG(DISTINCT pp.observaciones_produccion, ' | ') as observaciones_combinadas,
        ...
        $whereClause
        GROUP BY pp.id_venta, v.cliente, v.moneda, v.estado, u.nombre
        ORDER BY MIN(pp.fecha_asignacion) DESC
        LIMIT :limite OFFSET :inicio";
            ...
            error_log("Error obteniendo devoluciones agrupadas: " . $e->getMessage());
            return [];
        }
    }
    ...
    public function eliminarProductosExpedicionVenta($idVenta)
    {
        try {
            $sql = "DELETE FROM public.sist_ventas_productos_expedicion WHERE id_venta = :id_venta";
            $stmt = $this->conexion->prepare($sql);
            $stmt->bindParam(':id_venta', $idVenta, PDO::PARAM_INT);
            ...
            error_log("Eliminados $filasAfectadas registros de expedición para venta $idVenta");
            return $resultado;
        } catch (Exception $e) {
            error_log("Error eliminando productos de expedición: " . $e->getMessage());
            return false;
        }
    }
}
