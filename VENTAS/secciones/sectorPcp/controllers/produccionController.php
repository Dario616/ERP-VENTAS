...
/**
 * Controller para manejo de producción - CORREGIDO CON GESTIÓN DE RECETAS
 */
class ProduccionController
{
    ...
    /**
     * Procesar formulario de producción - CORREGIDO CON RECETAS Y JSON
     */
    public function procesarFormularioProduccion($idVenta, $datos)
    {
        ...
        try {
            switch ($accion) {
                ...
                case 'configurar_recetas':
                    $resultado = $this->configurarRecetasProductos($idVenta, $datos);
                    // ✅ NUEVO: Devolver JSON si es una solicitud AJAX
                    if (
            ...
            $resultado = ['error' => 'Error interno del servidor'];

            // ✅ NUEVO: Devolver JSON en caso de error si es AJAX
            if (
                !empty($_SERVER['HTTP_X_REQUESTED_WITH']) &&
                strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest'
            ) {
    ...
    /**
     * ✅ NUEVO: Configurar recetas para productos antes de emisión
     */
    public function configurarRecetasProductos($idVenta, $datos)
    {
        try {
            $productosSeleccionados = $datos['productos_seleccionados'] ?? [];

            if (empty($productosSeleccionados)) {
                return ['error' => 'Debe seleccionar al menos un producto'];
            }

            // Obtener recetas disponibles para cada producto
            $recetasDisponibles = [];
            foreach ($productosSeleccionados as $idProductoProduccion) {
                ...
                if ($producto) {
                    ...
                    if (!empty($recetas)) {
                        $recetasDisponibles[$idProductoProduccion] = [
                            'producto' => $producto,
                            'recetas' => $this->agruparRecetasPorVersion($recetas)
                        ];
                    }
                }
            }
            ...
            return [
                'success' => true,
                'tipo' => 'configurar_recetas',
                'productos_con_recetas' => $recetasDisponibles,
                'id_venta' => $idVenta
            ];
        } catch (Exception $e) {
            error_log("Error configurando recetas: " . $e->getMessage());
            return ['error' => 'Error al obtener recetas disponibles'];
        }
    }

    /**
     * ✅ NUEVO: Agrupar recetas por versión
     */
    private function agruparRecetasPorVersion($recetas)
    {
        $agrupadas = [];
        ...
        // Ordenar versiones descendentemente (más reciente primero)
    ...
    /**
     * ✅ NUEVO: Obtener recetas disponibles para un producto
     */
    public function obtenerRecetasProducto($idProductoProduccion)
    {
        try {
            $producto = $this->repository->obtenerInfoProductoProduccion($idProductoProduccion);
            if (!$producto) {
            ...
            $recetas = $this->repository->obtenerRecetasPorTipoProducto($producto['tipoproducto']);
            ...
            error_log("Error obteniendo recetas producto: " . $e->getMessage());
            return ['error' => 'Error al obtener recetas del producto'];
        }
    }

    /**
     * ✅ CORREGIDO: Procesar emisión de órdenes CON RECETAS - AÑADIDO JSON_DECODE
     */
    private function procesarEmisionOrdenesCorregido($idVenta, $datos)
    {
        try {
            ...
            // ✅ CORRECCIÓN: Decodificar JSON de recetas seleccionadas
            $recetasSeleccionadas = [];
            if (!empty($datos['recetas_seleccionadas'])) {
                $recetasJson = $datos['recetas_seleccionadas'];
                $recetasSeleccionadas = json_decode($recetasJson, true);

                if (json_last_error() !== JSON_ERROR_NONE) {
                    error_log("ERROR JSON DECODE: " . json_last_error_msg());
                    $recetasSeleccionadas = [];
                } else {
                    error_log("SUCCESS JSON DECODE - Recetas decodificadas: " . json_encode($recetasSeleccionadas));
                }
            }
            ...
            error_log("CONTROLLER - Recetas seleccionadas DESPUÉS DE DECODE: " . json_encode($recetasSeleccionadas));

            $resultado = $this->service->emitirOrdenesProduccionConRecetas(
                $idVenta,
                $productosCompletados,
                $recetasSeleccionadas, // ✅ AHORA ES ARRAY, NO STRING
                $observaciones,
                $_SESSION['id']
            );
            ...
            if ($resultado['success']) {
                ...
                foreach ($resultado['productos_emitidos'] as $producto) {
                    ...
                    $productosFormateados[] = [
                        ...
                        'cantidad_actualizada' => $producto['cantidad_actualizada'] ?? false,
                        'recetas_asignadas' => $producto['recetas_asignadas'] ?? [] // NUEVO
                    ];
                }
                ...
                return [
                    ...
                    'mensaje_adicional' => 'Órdenes emitidas con recetas configuradas'
                ];
            } else {
    ...
    public function verificarPermisos($accion, $idVenta = null)
    {
        ...
        switch ($accion) {
            ...
            case 'emitir':
            case 'devolver':
            case 'configurar_recetas': // NUEVO PERMISO
    ...
    /**
     * Validar venta para procesamiento en producción - MEJORADO
     */
    public function validarVentaParaProcesamiento($idVenta)
    {
        try {
            // Primero verificar si la venta existe
            $sqlExiste = "SELECT id, estado FROM public.sist_ventas_presupuesto WHERE id = :id";
            $stmt = $this->repository->getConexion()->prepare($sqlExiste);
            $stmt->bindParam(':id', $idVenta, PDO::PARAM_INT);
            $stmt->execute();
            $ventaBasica = $stmt->fetch(PDO::FETCH_ASSOC);

            if (!$ventaBasica) {
                throw new Exception('La venta #' . $idVenta . ' no existe en el sistema');
            }

            // Luego verificar el estado
            $estadosValidos = ['En Producción/Expedición', 'En Producción'];
            if (!in_array($ventaBasica['estado'], $estadosValidos)) {
                if ($ventaBasica['estado'] === 'Devuelto a PCP') {
                    throw new Exception('La venta #' . $idVenta . ' ya fue devuelta completamente a PCP y no requiere procesamiento');
                } else {
                    throw new Exception('La venta #' . $idVenta . ' no está en estado válido para procesamiento de producción (Estado actual: ' . $ventaBasica['estado'] . ')');
                }
            }

            // Finalmente obtener los datos completos
            $venta = $this->repository->obtenerVentaProduccion($idVenta);
            if (!$venta) {
                throw new Exception('Error interno al obtener datos de la venta #' . $idVenta);
            }
    ...
    /**
     * ✅ NUEVO: Obtener todas las recetas disponibles
     */
    public function obtenerTodasLasRecetas()
    {
        try {
            return $this->repository->obtenerTodasLasRecetas();
        } catch (Exception $e) {
    ...
    /**
     * ✅ NUEVO: Formatear información de receta para vista
     */
    public function formatearRecetaParaVista($receta)
    {
        return [
            'id' => $receta['id'],
            'nombre_receta' => $receta['nombre_receta'],
            'version_receta' => $receta['version_receta'],
            'tipo_producto' => $receta['tipo_producto'],
            'nombre_materia_prima' => $receta['nombre_materia_prima'],
            'cantidad_por_kilo' => number_format($receta['cantidad_por_kilo'], 3, ',', '.'),
            'es_materia_extra' => $receta['es_materia_extra'],
            'unidad_medida_extra' => $receta['unidad_medida_extra'],
            'activo' => $receta['activo'],
            'fecha_creacion' => date('d/m/Y', strtotime($receta['fecha_creacion'])),
            'usuario_creacion' => $receta['usuario_creacion']
        ];
    }
}
